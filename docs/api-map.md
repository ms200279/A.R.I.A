...
- `GET  /api/documents/[id]/comparisons` — 해당 문서가 **참여한** 비교 히스토리 목록(최신순). Query `limit`(기본 20, 상한 50). `lib/documents/list-document-comparisons`. 문서 미소유 404.
- `GET  /api/comparisons/[id]` — 비교 히스토리 단건 상세(`ComparisonHistoryDetailPayload`: 참여 문서 메타·`content` 스냅샷). `lib/documents/get-comparison-history`. 타인/없음 404.
