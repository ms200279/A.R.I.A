...
- `GET  /api/documents/[id]/comparisons` — 해당 문서가 **참여한** 비교 히스토리 목록. Query: `limit`(기본 20, 상한 50), `cursor`(base64url JSON: `created_at`+`id`+`sort`+`roleFilter`), `sort`=`created_at_desc`\|`created_at_asc`(기본 desc), `role_filter`=`all`\|`primary`\|`peer`\|`secondary`\|`unknown`(기본 all; 문서 맥락에서 해당 링크 행의 `anchor_role` 기준, `unknown`=null). 응답 `{ document_id, items, pageInfo, sort, role_filter }`. `lib/documents/listComparisonHistoriesPage` + `comparison-list-cursor`. DTO 항목에 `current_document_anchor_role` 등 기존 필드 유지. `invalid_cursor`\|`invalid_limit` 400, 문서 미소유 404. 감사: `document.read.comparisons`(실패 시 흐름 유지).
- `GET  /api/comparisons` — (전역) 로그인 사용자의 `comparison_histories` 목록. Query: `documentId`(선택, 있으면 해당 문서 맥락·소유 검증), `limit`, `cursor`, `sort`, `role_filter`(맥락 없으면 `all`만 적용). 응답 `{ items, pageInfo, sort, role_filter }`(문서 필터 시 `document_id` 추가). 동일 도메인 서비스 as `/api/documents/[id]/comparisons`.
